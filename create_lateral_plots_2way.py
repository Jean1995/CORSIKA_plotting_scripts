...
for count, p_name in enumerate(particles_names):
    print(count)
    plot_lateral_hist_ratio(BINS_R, [sublist[count] for sublist in C7_r_hists+C8_r_hists], labels, colors, f'{p_name}', r"distance to shower axis / m", ratio_lim=(-0.25, 0.25))
    plt.savefig(f"{OUTPUT_NAME}/lateral_{p_name}_r.png", dpi=300)

# E distribution

for count, p_name in enumerate(particles_names):
    print(count)
    plot_lateral_hist_ratio(BINS_E, [sublist[count] for sublist in C7_E_hists+C8_E_hists], labels, colors, f'{p_name}', r"energy on observation plane / GeV", ratio_lim=(-0.25, 0.25))
...
for count, p_name in enumerate(particles_names):
    print(count)
    plot_lateral_hist_ratio(BINS_T, [sublist[count] for sublist in C7_t_hists+C8_t_hists], labels, colors, f'{p_name}', r"arrival time delay / ns", xlog=XLOG_TIME, ratio_lim=(-0.25, 0,25))
    plt.savefig(f"{OUTPUT_NAME}/lateral_{p_name}_t.png", dpi=300)
