...
for count, p_name in enumerate(particles_names):
    print(count)
    plot_lateral_hist_ratio(BINS_R, [sublist[count] for sublist in C7_r_hists+C8_r_hists], labels, colors, f'{p_name}', r"distance to shower axis / m")
...
for count, p_name in enumerate(particles_names):
    print(count)
    plot_lateral_hist_ratio(BINS_T, [sublist[count] for sublist in C7_t_hists+C8_t_hists], labels, colors, f'{p_name}', r"arrival time delay / ns", xlog=XLOG_TIME)
